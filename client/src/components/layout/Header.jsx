      ...
      <nav className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          <div className="flex-shrink-0">
            <Link to="/" className="text-2xl font-bold text-gray-800">n<span className="text-indigo-800">raeL</span></Link>
          </div>
          ...
          <div className="flex items-center space-x-4">
            <Link to="/" className="text-gray-600 hover:text-indigo-600 transition-colors">Khóa học</Link>
            <Link to="/fav-courses" className="relative text-gray-600 hover:text-indigo-600 transition-colors">
              <HeartIcon />
              {favoriteIds.length > 0 && (
               <span className="absolute -top-2 -right-2 bg-indigo-600 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center">{favoriteIds.length}</span>
              )}
            </Link>
